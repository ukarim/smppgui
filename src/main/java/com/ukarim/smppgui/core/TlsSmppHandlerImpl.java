...
import com.ukarim.smppgui.protocol.*;
import com.ukarim.smppgui.protocol.pdu.*;
...
public class TlsSmppHandlerImpl implements SmppHandler {

    private static final int MAX_SHORT_MSG_LEN = 140;

    private final long TIMEOUT_MILLIS = TimeUnit.SECONDS.toMillis(30);

    private final AtomicInteger seqNumGenerator = new AtomicInteger();

    private final EventDispatcher eventDispatcher;
    private final TlsSmppClient smppClient;
    private Charset defaultCharset;

    public TlsSmppHandlerImpl(EventDispatcher eventDispatcher) {
        this.eventDispatcher = eventDispatcher;
        this.smppClient = new TlsSmppClient(this);
    }
    ...
    public void login(LoginModel loginModel) {
        ...
        try {
            if (!TlsSmppClient.isSSL(host, port)) {
                showErrorDialog("Server TLS: %s", "server does not speak TLS");
            } else {
            ...
            showErrorDialog("Unknown Host: %s", "confirm domain name");
        } catch (IOException e) {
            showErrorDialog("SSL ERROR: %s", e.getMessage());
        }
    }
